...
Esta es una resolucion extendida del ejercicio 
"Calculadora de Ahorros" proporcionada por los
alumnos @IberoFPV = Bruno Orgilles y @xilefe = Felix Serrano

ATENCION: 
El script hace uso de estructuras que en este 
momento aún no se han visto en clase, ya que
se darán en el tema 3.

Para poder ejecutar el codigo es posible que
necesites instalar termcolor. Para ello puedes
entrar en tu environment de trabajo e instalar
termcolor con pip escribiendo en tu terminal:
...
def ingresos(tipotrabajo, jornada):
    ...
    if tipotrabajo == 'Autonomo':
        ...
        facturacion_mensual = input('''¿Cual és su facturación mensual neta habitual?
        ''')
        facturacion_mensual = float(facturacion_mensual.replace(',','.'))
        horas_extra = 0
        num_pagas = 0
        jornada =''
        EU_semana = 0
        ...
        if tipotrabajo == 'Ajena':
            EU_horas = input(termcolor.colored('''¿A cuanto cobra la hora?
            ''','green'))
            EU_horas = float(EU_horas.replace(',','.'))
            facturacion_mensual = 0
            
            if jornada == 'Parcial':
                horas_semana = input(termcolor.colored('''¿Cuantas horas a la semana trabaja?
                ''', 'green'))
                horas_semana = float(horas_semana.replace(',','.')) 
            else:
                if jornada == 'Completa':
                    horas_semana = 40.0
        horas_extra = input(termcolor.colored('''¿Realiza horas extras ocasionalmente? cuantas por semana:
        ''', 'green'))
        horas_extra = float(horas_extra.replace(',','.'))
    ...
    while(True):
        ...
        if pagas_extra == 'si':
            num_pagas = input(termcolor.colored('''¿Cuantas pagas extra tiene al año?
            ''', 'green'))
            num_pagas = float(num_pagas.replace(',','.'))
            break
        else:
            if pagas_extra == 'no':
                num_pagas = int(0)
...
def gastos():
    ...
    # Temporizador
    time.sleep(2)
    alqui_hipo,agua,luz,gas,comida,movil = input(termcolor.colored('''- ¿Que tasa de alquiler o hipoteca paga? 
    ''', 'green')) , input(termcolor.colored('''- ¿Que tasa paga de agua?
    ''', 'green')) , input(termcolor.colored('''- ¿Que tasa paga de luz?
    ''', 'green')) , input(termcolor.colored('''- ¿Que tasa paga de gas?
    ''', 'green')) , input(termcolor.colored('''- ¿Cuanto gasta en comida?
    ''', 'green')) , input(termcolor.colored('''- ¿Que tasa paga de movil?
    ''', 'green'))
    alqui_hipo,agua,luz,gas,comida,movil = float(alqui_hipo.replace(',','.')),float(agua.replace(',','.')), \
    float(luz.replace(',','.')),float(gas.replace(',','.')),float(comida.replace(',','.')),float(movil.replace(',','.'))
    ...
    while(True):
        ...
        if g_anuales == 'si':
            g_anuales = input(termcolor.colored('''-¿A cuánto asciende la suma de estos gastos anuales?
            ''', 'green'))
            g_anuales = float(g_anuales.replace(',','.'))
            break
        else:
            if g_anuales == 'no':
                g_anuales = int(0)
    ...
    while(True):
        ...
        if g_extra == 'si':
            gastos_extra = input(termcolor.colored('''- ¿Cuanto dinero gasta en extras?
            ''', 'green'))
            gastos_extra = float(gastos_extra.replace(',','.'))
            break
        else:
            if g_extra == 'no':
                gastos_extra = int(0)
